...
#get_dated_paths(X, Y, P, D) <= get_dated_paths(X, Z, P2, D) & knows(Z, Y, D2) & (X != Y) & X._not_in(P2) & Y._not_in(P2) & (P == P2 + [Z,D2]) & (D > D2) & (D2 >= date_board_decision) & (D2 <= date_shares_bought)
get_dated_paths(X, Y, P, D) <= knows(X, Y, D) & (P == []) & (D >= date_board_decision) & (D <= date_shares_bought)
#get_dated_paths(X, Y, P, D2) <= knows(X, Y, D2) & (P == []) & (D2 >= date_board_decision) & (D2 <= date_shares_bought)
...
"""The tip was probably from Eder Eva as she is the only boardmember that had indirect contact with the suspect in the given period
"""
